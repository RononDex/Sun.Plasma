...
namespace Sun.Core.SelfUpdating
{
    ...
    public class Application : INotifyPropertyChanged
    {
        ...
        public XElement GetChangelogForVersion(Version version)
        {
            ...
            var changeLogEntry = this.ChangeLog.Root.Elements("Version").Where(entry => entry.Attribute(XName.Get("Version")).Value == string.Format("{0}.{1}.{2}", version.Major, version.Minor, version.Build)).FirstOrDefault();
        ...
        /// <param name="version"></param>
        /// <returns></returns>
        public List<string> GetAllDeletedFilesForVersion(Version version)
        {
            if (this.ChangeLog == null)
                return null;
            ...
            var deletedFiles = versionElement.Elements("DeletedFile");

            var result = new List<string>();
            foreach (var deletedFile in deletedFiles)
            {
                result.Add(deletedFile.Value);
            }
        ...
        public void AddVersionToChangelog(Version version, List<string> bugFixes, List<string> newStuff, List<string> deletedFiles)
        {
            ...
            newVersionElement.Add(new XAttribute(XName.Get("Version"), string.Format("{0}.{1}.{2}", version.Major, version.Minor, version.Build)));
            ...
            // Add deletedFiles entries
            foreach (var deletedFile in deletedFiles)
            {
                var element = new XElement(XName.Get("DeletedFile"), deletedFile);
                newVersionElement.Add(element);
            }
